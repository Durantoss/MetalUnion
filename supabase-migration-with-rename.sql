-- MetalUnion Database Migration Script for Supabase (With Sessions Table Rename)
-- This script creates all tables with proper UUID types and renames sessions to user_sessions
...
-- Index for the new table (user_sessions) – column exists by definition
...
-- User Sessions table RLS policies (UPDATED for renamed table)
...
-- FIXED: Direct messages insert policy with proper UUID syntax and sender_id check
...
-- MIGRATION OPERATIONS: Handle existing sessions table modifications
-- This should only be run if you can afford brief table locks

/* Fixed migration script – ensure each DO block is properly terminated */

/* 1️⃣ Add user_id column to existing sessions table (if it exists) */
DO $$
BEGIN
   IF EXISTS (
...
/* 2️⃣ Rename the table to user_sessions (if it hasn't been renamed yet) */
DO $$
BEGIN
   IF EXISTS (
      SELECT 1 FROM information_schema.tables
      ...
      /* Rename old index names to match the new table name */
...
/* 3️⃣ Ensure the new table also has the column & index (in case the table existed already without them) */
DO $$
BEGIN
   IF EXISTS (
      SELECT 1 FROM information_schema.tables
...
-- Query to check existing policies on the renamed table (for verification)
-- This will help verify that all policies have been properly updated
SELECT 
    policyname, 
    tablename, 
    schemaname,
...
-- Success message
SELECT 'MetalUnion database schema created successfully with user_sessions table and all RLS policies updated!' as status;
